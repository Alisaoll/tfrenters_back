...
function FileProgress(file, targetID) {
    ...
    if (!this.fileProgressWrapper.length) {
        ...
        progressText.addClass('progressName').prepend('<div style="word-break:break-all;">' + file.name + '</div>');
...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    if (res.url) {
        url = res.url;
        str = "<div style='word-break:break-all;'><strong>Link:</strong><a href=" + res.url + " target='_blank' > " + res.url + "</a></div>" +
            "<div style='word-break:break-all;' class=hash><strong>Hash:</strong>" + res.hash + "</div>";
    } else {
        var domain = up.getOption('domain');
        url = domain + encodeURI(res.key);
        var link = domain + res.key;
        str = "<div style='word-break:break-all;'><strong>Link:</strong><a href=" + url + " target='_blank' > " + link + "</a></div>" +
            "<div style='word-break:break-all;' class=hash><strong>Hash:</strong>" + res.hash + "</div>";
    }
